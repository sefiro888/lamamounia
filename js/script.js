...
if(form){const date=document.querySelector('#date');const now=new Date();const today=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`;date.min=today;const messages={name:'Escribe un nombre de al menos 2 caracteres.',phone:'Introduce un teléfono válido (entre 9 y 15 cifras).',date:'Elige una fecha de hoy o posterior.',time:'Indica la hora a la que te gustaría venir.'};for(const id of Object.keys(messages)){document.getElementById(id).setAttribute('aria-describedby',id+'-error')}
form.addEventListener('submit',e=>{e.preventDefault();let first=null;for(const id of Object.keys(messages)){const input=document.getElementById(id);let valid=input.checkValidity();if(id==='name')valid=input.value.trim().length>=2;if(id==='phone')valid=/^[+\d\s().-]+$/.test(input.value)&&input.value.replace(/\D/g,'').length>=9&&input.value.replace(/\D/g,'').length<=15;if(id==='date')valid=!!input.value&&input.value>=today;document.getElementById(id+'-error').textContent=valid?'':messages[id];input.setAttribute('aria-invalid',String(!valid));if(!valid&&!first)first=input}const status=document.querySelector('#form-status');if(first){status.textContent='Revisa los campos señalados para continuar.';first.focus();return}/* Demo local: no datos enviados ni almacenados. Requiere backend para reservas reales. */status.textContent='Prueba completada. No se ha enviado ninguna solicitud ni reservado una mesa. Para confirmar tu visita, llama al 620 26 20 19 o consulta OpenTable.';status.focus()})}
